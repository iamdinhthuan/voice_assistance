...
def main():
    ...
    if use_cuda:
        ...
        logging.info('CUDA not available or gpu=-1, fallback to CPU')
    if world_size > 1:
        if not use_cuda:
            raise ValueError('Distributed training requires CUDA-enabled devices')
    ...
    loader_conf = dict(batch_size=None,
                       pin_memory=args.pin_memory,
                       num_workers=args.num_workers)
    if args.num_workers > 0:
        loader_conf['prefetch_factor'] = args.prefetch

    train_data_loader = DataLoader(train_dataset, **loader_conf)
    cv_data_loader = DataLoader(cv_dataset, **loader_conf)
    ...
    if rank == 0:
        ...
        try:
            ...
            logging.warning('Tensorboard writer disabled: {}'.format(ex))
            writer = None
